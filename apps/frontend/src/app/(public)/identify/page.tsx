...
function classifyDisplayLabel(dangerAssessment?: string | null) {
  switch (dangerAssessment) {
    case 'HIGH_RISK':
      return { label: 'LIKELY VENOMOUS', pill: 'border-destructive/40 bg-destructive/15 text-destructive', bar: 'bg-destructive' };
    case 'LOW_RISK':
      return { label: 'LIKELY NON-VENOMOUS', pill: 'border-success/40 bg-success/15 text-success', bar: 'bg-success' };
    default:
      return { label: 'IDENTIFICATION UNCERTAIN', pill: 'border-warning/40 bg-warning/15 text-warning', bar: 'bg-warning' };
  }
}

export default function IdentifyPage() {
  const [preview, setPreview] = useState<{ url: string; name: string; file?: File } | null>(null);
  const [state, setState] = useState<'idle' | 'uploading' | 'scanning' | 'done'>('idle');
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<IdentificationResult | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  const { upload, isUploading, progress } = useMediaUpload();
  const [identifySnake, { loading: identifying }] = useMutation<{ identifySnake: IdentificationResult }>(IDENTIFY_SNAKE_MUTATION);
  ...
  async function identify() {
    ...
    try {
      ...
      const alternativeMatches = (mlResult.top_species ?? [])
        ...
        .slice(0, 3)
        .map((sp: { common_name?: string; scientific_name?: string; confidence?: number; venomous?: boolean }) => ({
          confidence: sp.confidence ?? 0,
          reasoning: sp.venomous ? 'Venomous species' : 'Non-venomous species',
          species: {
  ...
  const riskText = result?.dangerAssessment === 'HIGH_RISK'
    ...
    <div>
      <section className="relative px-5 py-20 lg:py-28 text-center overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="mx-auto mt-5 max-w-xl text-lg leading-relaxed text-muted-foreground">
            Take a clear photo from a safe distance. Our AI will estimate the species and guide the next safest step.
          </p>
          <div className="mt-6 flex flex-wrap items-center justify-center gap-6 text-sm text-muted-foreground">
            <span className="flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-accent" /> Verified snake knowledge base
            </span>
            <span className="flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-accent" /> Safety-first AI analysis
            </span>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-6xl px-5 py-12">
        <div className="grid gap-8 lg:grid-cols-2">
          <div>
            <h2 className="flex items-center gap-2 text-lg font-bold">
              <Upload className="h-4 w-4 text-primary" /> Identify a Snake
            </h2>
              ...
              {preview ? (
                ...
                >
                  <span>
                    ...
                    <span className="mt-4 block font-semibold">Take a clear photo from a safe distance</span>
                    <span className="mt-1 block text-xs text-muted-foreground">
                      Do not approach the snake. Upload a JPG, PNG, or WEBP image under 10MB.
                    </span>
            ...
            <div className="mt-4 flex gap-2">
              <Button
                ...
                size="lg"
                onClick={identify}
                disabled={!preview || state === 'uploading' || state === 'scanning' || identifying || isUploading}
              >
                {state === 'uploading' || state === 'scanning' || identifying || isUploading ? (
                  <Loader2 className="mr-1.5 h-4 w-4 animate-spin" />
                ) : (
                  <Sparkles className="mr-1.5 h-4 w-4" />
                )}
                {state === 'uploading' ? 'Uploading image...' : state === 'scanning' ? 'Analyzing snake...' : 'Identify Snake'}
              </Button>
              {preview && (
              ...
              <div className="mt-4 space-y-5">
                <div className="rounded-2xl border border-accent/40 bg-accent/10 backdrop-blur-sm shadow-md p-6">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="text-xl font-bold">{result.species?.name || 'Uncertain identification'}</h3>
                      <p className="text-sm italic text-muted-foreground">
                        {result.species?.scientificName || 'Not confidently matched'}
                      </p>
                    </div>
                    <span className={`rounded-full border px-3 py-1 text-[10px] font-bold uppercase tracking-wider ${meta.pill}`}>
                      {meta.label}
                    </span>
                  </div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-secondary">
                      <div className={meta.bar} style={{ width: `${confidencePercent}%`, height: '100%' }} />
                    </div>
                  </div>

                  <p className="mt-4 text-sm leading-relaxed">{riskText}</p>
                  <p className="mt-3 text-xs text-muted-foreground">
                    Provider: <span className="font-semibold text-primary">{result.provider || 'Vision AI'}</span>
                    {' · '}
                    Model: <span className="font-semibold text-primary">{result.model || 'vision-ai'}</span>
                  </p>
                </div>
                ...
                {result.alternativeMatches && result.alternativeMatches.length > 0 && (
                  <div className="rounded-2xl border border-border/30 bg-background/60 p-5">
                    ...
                    <ul className="mt-3 space-y-2 text-sm">
                      {result.alternativeMatches.slice(0, 3).map((match, index) => (
                        <li key={`${match.species?.scientificName ?? 'match'}-${index}`} className="flex items-center justify-between gap-3">
                          <span>{match.species?.commonName || match.species?.name || 'Other species'}</span>
                          <span className="font-mono text-xs text-muted-foreground">{Math.round((match.confidence ?? 0) * 100)}%</span>
                        </li>
                      ))}
                    </ul>
                  </div>
        ...
        <div className="mt-10 rounded-xl border border-warning/40 bg-warning/10 p-5">
          ...
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
            This AI assistance is not a guarantee of species identification. Keep your distance, do not approach the snake,
            and contact a trained rescuer if there is any uncertainty or risk.
          </p>
        </div>
      </div>
    </div>
