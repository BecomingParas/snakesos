        ...
        <div className="glass-card rounded-3xl p-8 border border-white/10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div><label className="text-sm text-emerald-400 font-medium mb-2 block">Full Name *</label><input value={form.name} onChange={e => update('name', e.target.value)} placeholder="Your full name" className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-white placeholder-gray-400" /></div>
              <div><label className="text-sm text-emerald-400 font-medium mb-2 block">Phone / Contact *</label><input value={form.contact} onChange={e => update('contact', e.target.value)} placeholder="98XXXXXXXX" type="tel" className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-white placeholder-gray-400" /></div>
              <div><label className="text-sm text-emerald-400 font-medium mb-2 block">Address</label><input value={form.address} onChange={e => update('address', e.target.value)} placeholder="Your home address" className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-white placeholder-gray-400" /></div>
              <div><label className="text-sm text-emerald-400 font-medium mb-2 block">Municipality</label><select value={form.municipality} onChange={e => update('municipality', e.target.value)} className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-white appearance-none">{MUNICIPALITIES.map(m => <option key={m} value={m} className="bg-[#1a2b2f]">{m}</option>)}</select></div>
              ...
              <div><label className="text-sm text-emerald-400 font-medium mb-2 block">Experience Level</label><select value={form.experience} onChange={e => update('experience', e.target.value)} className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-white appearance-none">{EXPERIENCE_LEVELS.map(l => <option key={l} value={l} className="bg-[#1a2b2f]">{l}</option>)}</select></div>
              <div><label className="text-sm text-emerald-400 font-medium mb-2 block">Vehicle</label><select value={form.vehicle} onChange={e => update('vehicle', e.target.value)} className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-white appearance-none">{VEHICLES.map(v => <option key={v} value={v} className="bg-[#1a2b2f]">{v}</option>)}</select></div>
              <div><label className="text-sm text-emerald-400 font-medium mb-2 block">Available Time</label><select value={form.availableTime} onChange={e => update('availableTime', e.target.value)} className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-white appearance-none">{AVAILABLE_TIMES.map(t => <option key={t} value={t} className="bg-[#1a2b2f]">{t}</option>)}</select></div>
              <div><label className="text-sm text-emerald-400 font-medium mb-2 block">Emergency Availability</label><select value={form.emergencyAvailability} onChange={e => update('emergencyAvailability', e.target.value)} className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-white appearance-none"><option value="Yes" className="bg-[#1a2b2f]">Yes — I'm available for emergencies</option><option value="No" className="bg-[#1a2b2f]">No — Scheduled shifts only</option></select></div>
            </div>
            <div><label className="text-sm text-emerald-400 font-medium mb-2 block">Skills / Background (Optional)</label><textarea value={form.skills} onChange={e => update('skills', e.target.value)} placeholder="e.g. First aid certified, wildlife photography, veterinary student..." rows={3} className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-white placeholder-gray-400 resize-none" /></div>
